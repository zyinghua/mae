...
python -m torch.distributed.launch --nproc_per_node=4 main_pretrain.py \
  --batch_size 64 \
  --model mae_vit_small_patch16_enc384emb \
  --mask_ratio 0.75 \
  --epochs 200 \
  --warmup_epochs 40 \
  --blr 1.5e-4 \
  ...
  --output_dir /root/scratch/mae-base-pretrain-output-enc384emb \
...
python -m torch.distributed.launch --nproc_per_node=4 main_linprobe.py \
    ...
    --blr 2e-3 \
    --weight_decay 0.0 \
    --output_dir /root/scratch/mae-base-linprobe-output-enc384emb